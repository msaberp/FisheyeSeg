...
class MyTransform(object):
    ...
    def _rand_crop(self, image, annot):
        ...
        crop_image = image[
            row_start : row_start + new_rows, col_start : col_start + new_cols
        ]
        crop_annot = annot[
            row_start : row_start + new_rows, col_start : col_start + new_cols
        ]
    ...
    def _fish_scale(self, image, annot):
        borderValue = 20
        rate = (
            random.random() * (self._fish_scale_range[1] - self._fish_scale_range[0])
        ...
        annot = cv2.resize(
            annot, None, fx=rate, fy=rate, interpolation=cv2.INTER_NEAREST
        )
        if rate < 1:
            ...
            dst_image[
                row_start : row_start + annot.shape[0],
                col_start : col_start + annot.shape[1],
            ] = image
            dst_annot[
                row_start : row_start + annot.shape[0],
                col_start : col_start + annot.shape[1],
    ...
    def __call__(self, image, annot):
        if self._RAND_CROP:
            ...
            if self._NORMAL_SCALE:
                scale_rate = (
                    random.random() * (self._scale_range[1] - self._scale_range[0])
                    + self._scale_range[0]
                )
        ...
        if self._NORMAL_TRANSLATE:
            x_shift = (
                random.random() * (self._trans_range[1] - self._trans_range[0])
                + self._trans_range[0]
            )
            y_shift = (
                random.random() * (self._trans_range[1] - self._trans_range[0])
            ...
            dst_annot = cv2.warpAffine(
                dst_annot, M, sz, flags=cv2.INTER_NEAREST, borderValue=20
            )
...
def val(model, dataloader, ignore_index=19, is_print=False):
    ...
    with torch.no_grad():
        for i, (image, annot) in enumerate(dataloader):
            ...
            predict_onehot = torch.zeros(
                predict.size(0), Config.class_num, predict.size(2), predict.size(3)
            ).cuda()
            predict_onehot = predict_onehot.scatter(1, predict, 1).float()
            target.unsqueeze_(1)
            target_onehot = torch.zeros(
                target.size(0), Config.class_num, target.size(2), target.size(3)
            ).cuda()
            target_onehot = target_onehot.scatter(1, target, 1).float()

            mask = (1 - target_onehot[:, ignore_index, :, :]).view(
            ...
            pix_num_TPFP += torch.sum(predict_onehot, dim=(0, 2, 3)).cpu().numpy()
            pix_num_TPFN += torch.sum(target_onehot, dim=(0, 2, 3)).cpu().numpy()
    ...
    if is_print:
        print("==================RESULT====================")
        print("Mean precision:", mean_precision, "; Mean precision 19:", m_precision_19)
...
def val_distortion(model, dataloader, ignore_index=19, is_print=False):
    ...
    for i in range(Config.fish_size[0]):
        for j in range(Config.fish_size[1]):
            if (i - Config.fish_size[0] / 2) ** 2 + (
                j - Config.fish_size[1] / 2
            ) ** 2 < Config.mask_radius ** 2:
...
def final_eval():
    ...
    for i in range(len(valid_path)):
        validation_set = CityScape(
            Config.data_dir + valid_path[i], Config.data_dir + valid_annot[i]
        )
...
def one_eval(model, ckpt_path, valid_path, valid_annot_path, is_distortion=False):
    ...
    model.eval()
    validation_set = CityScape(
        Config.data_dir + valid_path, Config.data_dir + valid_annot_path
    )
    validation_loader = DataLoader(
        validation_set, batch_size=Config.val_batch_size, shuffle=False
    )
...
def train():
    ...
    train_set = CityScape(
        Config.train_img_dir, Config.train_annot_dir, transform=train_transform
    )
    ...
    for epoch in range(start_epoch, Config.max_epoch):
        ...
        for i, (image, annot) in enumerate(train_loader):
            ...
            if i % 20 == 0:
                ...
                writer.add_image(
                    "Images/original_image", image[0], global_step=global_step
                )
                writer.add_image(
...
def test_eyefish_transform():
    transform = MyTransform(Config.f, Config.fish_size)
    transform.set_ext_params(Config.ext_param)
    transform.set_ext_param_range(Config.ext_range)
    if Config.rand_f:
        transform.rand_f(f_range=Config.f_range)
    if Config.rand_ext:
        transform.rand_ext_params()
    transform.set_bkg(bkg_label=20, bkg_color=[0, 0, 0])
    transform.set_crop(rand=Config.crop, rate=Config.crop_rate)
